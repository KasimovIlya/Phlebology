...
class ImageDataGenerator(object):
    ...
    def __init__(self,
                 contrast_stretching=False,  # AUG
                 histogram_equalization=False,  # AUG
                 adaptive_equalization=False,  # AUG
                 ...
                 dtype='float32'):
        self.contrast_stretching = contrast_stretching  # AUG
        self.adaptive_equalization = adaptive_equalization  # AUG
        self.histogram_equalization = histogram_equalization  # AUG
    ...
    def random_transform(self, x, seed=None):
        ...
        x = self.apply_transform(x, params) # AUG
        if self.contrast_stretching:  # AUG
            if np.random.random() < 0.5:  # AUG
                p2, p98 = np.percentile(x, (2, 98))  # AUG
                x = exposure.rescale_intensity(x, in_range=(p2, p98))  # AUG

        if self.adaptive_equalization:  # AUG
            if np.random.random() < 0.5:  # AUG
                x = exposure.equalize_adapthist(x, clip_limit=0.03)  # AUG

        if self.histogram_equalization:  # AUG
            if np.random.random() < 0.5:  # AUG
                x = exposure.equalize_hist(x)  # AUG
        return x
